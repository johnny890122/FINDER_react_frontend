...
export const HintDialog = ({ open, onConfirm, step }) => (
  <Dialog
    open={open}
    title={`成功移除編號為 #${nodeIdsToBeRemoved[step]?.id} 的點！`}
    type={DialogTypes.CONFIRM}
    confirmText={step === 2 ? '準備好了，開始遊戲！' : '繼續移除下一個點'}
    onConfirm={onConfirm}
  >
    您已成功移除編號為 #{nodeIdsToBeRemoved[step]?.id} 的點，您可以觀察到它從網絡中消失了！
    <StyledImagesContainer>
      <StyledImage src={nodeIdsToBeRemoved[step]?.beforeImage} alt="before" />
      <EastIcon />
      <StyledImage src={nodeIdsToBeRemoved[step]?.afterImage} alt="after" />
    </StyledImagesContainer>
